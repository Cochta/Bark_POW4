  ...
  for (auto* client : clients) {
    ...
    if (client->acknowledged) {
      ...
      if (client->socket->send(*packet) == sf::Socket::Done) {
        ...
        // If the packet is a AcknowledgementPacket, do not wait for an
        // acknowledgement
        if (PacketManager::GetPacketType(*packet) ==
    ...
    } else if (client->ackClock.getElapsedTime().asMilliseconds() >
               Client::ACK_TIMEOUT) {
      // Resend the packet
      if (client->socket->send(*client->packetWaitingForAcknowledgement) ==
          sf::Socket::Done) {
        client->ackClock.restart();
      } else {
