...
void World::Update(const float deltaTime) noexcept {
  ...
  UpdateQuadTreeCollisions(QT.Nodes[0]);
}
      ...
      for (std::size_t j = i + 1; j < node.ColliderRefAabbs.size(); ++j) {
        ...
        if (!col2.IsTrigger && !col1.IsTrigger)  // Physical collision
        {
          if (Overlap(col1, col2)) {
            Contact contact;
            contact.CollidingBodies[0] = {&GetBody(col1.BR), &col1};
            contact.CollidingBodies[1] = {&GetBody(col2.BR), &col2};
            contact.Resolve();
...
[[nodiscard]] bool World::Overlap(const Collider& colA,
  ...
  switch (ShapeA) {
    case Math::ShapeType::Circle: {
      Math::CircleF circle =
          std::get<Math::CircleF>(colA.Shape) + GetBody(colA.BR).Position;
      switch (ShapeB) {
        case Math::ShapeType::Circle:
    ...
    case Math::ShapeType::Rectangle: {
      Math::RectangleF rect =
          std::get<Math::RectangleF>(colA.Shape) + GetBody(colA.BR).Position;
      switch (ShapeB) {
        case Math::ShapeType::Circle:
    ...
    case Math::ShapeType::Polygon: {
      Math::PolygonF pol =
          std::get<Math::PolygonF>(colA.Shape) + GetBody(colA.BR).Position;
      switch (ShapeB) {
        case Math::ShapeType::Circle:
